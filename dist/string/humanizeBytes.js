...
// lib/string/humanizeBytes.mjs
var humanizeBytes_exports = {};
__export(humanizeBytes_exports, {
  default: () => humanizeBytes
});
module.exports = __toCommonJS(humanizeBytes_exports);
...
function humanizeBytes(val, options = {}) {
  ...
  return humanizeNumber(val, {
    ...
    separator: has_opts && typeof options.separator === "string" && options.separator.trim().length > 0 ? options.separator : ".",
    precision: has_opts && Number.isInteger(options.precision) && options.precision >= 0 ? options.precision : 2,
    units: has_opts && Array.isArray(options.units) && options.units.length > 0 ? options.units.filter(isNotEmptyString) : [" bytes", " KB", " MB", " GB", " TB", " PB", " EB", " ZB", " YB"],
    divider: 1024,
    real: true
  });
}
