...
describe('Number - isIntegerAboveOrEqual', () => {
    ...
    it('Treat numeric values above min correctly', () => {
        for (const el of [
            ...
            expect(isIntegerAboveOrEqual(el[0], el[1])).toBeTruthy();
        }
    });
    ...
    it('Treat numeric values below min as false', () => {
        for (const el of [
            ...
            expect(isIntegerAboveOrEqual(el[0], el[1])).toBe(false);
        }
    });

    it('Treat numeric values at min as true', () => {
        for (const el of [0, -100, 1]) {
            expect(isIntegerAboveOrEqual(el, el)).toBeTruthy();
        }
    });
});
