...
var _is4 = _interopRequireDefault(require("../string/is"));
...
function humanizeNumber(val) {
  ...
  var OPTS = {
    delim: (0, _is3["default"])(options) && (0, _is4["default"])(options.delim) ? options.delim : ',',
    separator: (0, _is3["default"])(options) && (0, _isNotEmpty2["default"])(options.separator) ? options.separator : '.',
    precision: (0, _is3["default"])(options) && (0, _isIntegerAboveOrEqual["default"])(options.precision, 0) ? options.precision : 2,
    units: (0, _is3["default"])(options) && ((0, _isNotEmpty["default"])(options.units) || options.units === !1) ? options.units ? options.units.filter(_is4["default"]) : !1 : ['', 'k', 'm', 'b', 't', 'q'],
    divider: (0, _is3["default"])(options) && (0, _isIntegerAboveOrEqual["default"])(options.divider, 2) ? options.divider : 1000,
    real: (0, _is3["default"])(options) && (0, _is["default"])(options.real) ? options.real : !1
  };
  if (!((0, _is2["default"])(val) || (0, _isNotEmpty2["default"])(val))) {
    return "0".concat(OPTS.units.length > 0 ? OPTS.units[0] : '');
  }
  var normalized;
  if (OPTS.real) {
    normalized = parseInt((0, _is4["default"])(val) ? val.trim() : val) || 0;
  } else {
    normalized = parseFloat((0, _is4["default"])(val) ? val.trim() : val) || 0;
  }
  if (!(0, _is2["default"])(normalized) || normalized === 0) {
    return "0".concat(OPTS.units.length > 0 ? OPTS.units[0] : '');
  }
