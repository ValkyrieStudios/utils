...
describe('Number - isAboveOrEqual', () => {
    ...
    it('Treat numeric values above min correctly', () => {
        for (const el of [
            ...
            expect(isAboveOrEqual(el[0], el[1])).toBeTruthy();
        }
    });
    ...
    it('Treat numeric values below min as false', () => {
        for (const el of [
            ...
            expect(isAboveOrEqual(el[0], el[1])).toBe(false);
        }
    });

    it('Treat numeric values at min as true', () => {
        for (const el of [0, -100, 1, 0, 0.56, 0.89]) {
            expect(isAboveOrEqual(el, el)).toBeTruthy();
        }
    });
});
