...
describe("Number", () => {
    ...
    describe("isNumberBelowOrEqual", () => {
        it ('not see a string as a number', () => {
            let vals = fnStringValues();
            for (let el of vals) expect(isNumberBelowOrEqual(el, 0)).to.eql(false);
        });
        ...
        it ('treat numeric values below max correctly', () => {
            ...
            for (let el of vals) expect(isNumberBelowOrEqual(el[0], el[1])).to.eql(true);
        });
        ...
        it ('treat numeric values above max as false', () => {
            ...
            for (let el of vals) expect(isNumberBelowOrEqual(el[0], el[1])).to.eql(false);
        });

        it ('treat numeric values at max as true', () => {
            let vals = [0, -100, 1, 0, .56, .89];
            for (let el of vals) expect(isNumberBelowOrEqual(el, el)).to.eql(true);
        });

        it ('not see a boolean as a number below or equal', () => {
            let vals = fnBooleanValues();
            for (let el of vals) expect(isNumberBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a regex as a number below or equal', () => {
            let vals = fnRegexValues();
            for (let el of vals) expect(isNumberBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see an object as a number below or equal', () => {
            let vals = fnObjectValues();
            for (let el of vals) expect(isNumberBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a nullable as a number below or equal', () => {
            let vals = fnNullables();
            for (let el of vals) expect(isNumberBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a date as a number below or equal', () => {
            let vals = fnDateValues();
            for (let el of vals) expect(isNumberBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see an array as a number below or equal', () => {
            let vals = fnArrayValues();
            for (let el of vals) expect(isNumberBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a function as a number below or equal', () => {
            let vals = fnFunctionValues();
            for (let el of vals) expect(isNumberBelowOrEqual(el, 0)).to.eql(false);
        });
        ...
        it ('not see a boolean as a number below or equal', () => {
            let vals = fnBooleanValues();
            for (let el of vals) expect(isNumberBelowOrEqual(0, el)).to.eql(false);
        });
    ...
    describe("isNumberAboveOrEqual", () => {
        it ('not see a string as a number', () => {
            let vals = fnStringValues();
            for (let el of vals) expect(isNumberAboveOrEqual(el, 0)).to.eql(false);
        });
        ...
        it ('treat numeric values above min correctly', () => {
            ...
            for (let el of vals) expect(isNumberAboveOrEqual(el[0], el[1])).to.eql(true);
        });
        ...
        it ('treat numeric values below min as false', () => {
            ...
            for (let el of vals) expect(isNumberAboveOrEqual(el[0], el[1])).to.eql(false);
        });

        it ('treat numeric values at min as true', () => {
            let vals = [0, -100, 1, 0, .56, .89];
            for (let el of vals) expect(isNumberAboveOrEqual(el, el)).to.eql(true);
        });

        it ('not see a boolean as a number above or equal', () => {
            let vals = fnBooleanValues();
            for (let el of vals) expect(isNumberAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a regex as a number above or equal', () => {
            let vals = fnRegexValues();
            for (let el of vals) expect(isNumberAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see an object as a number above or equal', () => {
            let vals = fnObjectValues();
            for (let el of vals) expect(isNumberAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a nullable as a number above or equal', () => {
            let vals = fnNullables();
            for (let el of vals) expect(isNumberAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a date as a number above or equal', () => {
            let vals = fnDateValues();
            for (let el of vals) expect(isNumberAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see an array as a number above or equal', () => {
            let vals = fnArrayValues();
            for (let el of vals) expect(isNumberAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a function as a number above or equal', () => {
            let vals = fnFunctionValues();
            for (let el of vals) expect(isNumberAboveOrEqual(el, 0)).to.eql(false);
        });
        ...
        it ('not see a boolean as a number above or equal', () => {
            let vals = fnBooleanValues();
            for (let el of vals) expect(isNumberAboveOrEqual(0, el)).to.eql(false);
        });
    ...
    describe("isIntegerBelow", () => {
        ...
        it ('treat float values as false', () => {
            let vals = [.56, .89];
            for (let el of vals) expect(isIntegerBelow(el, 1)).to.eql(false);
        });
    ...
    describe("isIntegerBelowOrEqual", () => {
        it ('not see a string as an integer below', () => {
            let vals = fnStringValues();
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 0)).to.eql(false);
        });
        ...
        it ('treat numeric values below max correctly', () => {
            ...
            for (let el of vals) expect(isIntegerBelowOrEqual(el[0], el[1])).to.eql(el[2]);
        });
        ...
        it ('treat float values as false', () => {
            let vals = [.56, .89];
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 1)).to.eql(false);
        });

        it ('treat float values at max as false', () => {
            let vals = [.56, .89];
            for (let el of vals) expect(isIntegerBelowOrEqual(el, el)).to.eql(false);
        });

        it ('treat integer values at max as true', () => {
            let vals = [0, -100, 1, 983420];
            for (let el of vals) expect(isIntegerBelowOrEqual(el, el)).to.eql(true);
        });

        it ('not see a boolean as an integer below or equal', () => {
            let vals = fnBooleanValues();
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a regex as an integer below or equal', () => {
            let vals = fnRegexValues();
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see an object as an integer below or equal', () => {
            let vals = fnObjectValues();
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a nullable as an integer below or equal', () => {
            let vals = fnNullables();
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a date as an integer below or equal', () => {
            let vals = fnDateValues();
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see an array as an integer below or equal', () => {
            let vals = fnArrayValues();
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a function as an integer below or equal', () => {
            let vals = fnFunctionValues();
            for (let el of vals) expect(isIntegerBelowOrEqual(el, 0)).to.eql(false);
        });
        ...
        it ('not see a boolean as an integer below or equal', () => {
            let vals = fnBooleanValues();
            for (let el of vals) expect(isIntegerBelowOrEqual(0, el)).to.eql(false);
        });
    ...
    describe("isIntegerAbove", () => {
        ...
        it ('treat float values at min as false', () => {
            let vals = [.56, .89];
    ...
    describe("isIntegerAboveOrEqual", () => {
        it ('not see a string as an integer above', () => {
            let vals = fnStringValues();
            for (let el of vals) expect(isIntegerAboveOrEqual(el, 0)).to.eql(false);
        });
        ...
        it ('treat numeric values above min correctly', () => {
            ...
            for (let el of vals) expect(isIntegerAboveOrEqual(el[0], el[1])).to.eql(el[2]);
        });
        ...
        it ('treat numeric values at min as true', () => {
            let vals = [0, -100, 1, 0];
            for (let el of vals) expect(isIntegerAboveOrEqual(el, el)).to.eql(true);
        });

        it ('not see a boolean as an integer above or equal', () => {
            let vals = fnBooleanValues();
            for (let el of vals) expect(isIntegerAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a regex as an integer above or equal', () => {
            let vals = fnRegexValues();
            for (let el of vals) expect(isIntegerAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see an object as an integer above or equal', () => {
            let vals = fnObjectValues();
            for (let el of vals) expect(isIntegerAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a nullable as an integer above or equal', () => {
            let vals = fnNullables();
            for (let el of vals) expect(isIntegerAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a date as an integer above or equal', () => {
            let vals = fnDateValues();
            for (let el of vals) expect(isIntegerAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see an array as an integer above or equal', () => {
            let vals = fnArrayValues();
            for (let el of vals) expect(isIntegerAboveOrEqual(el, 0)).to.eql(false);
        });

        it ('not see a function as an integer above or equal', () => {
            let vals = fnFunctionValues();
            for (let el of vals) expect(isIntegerAboveOrEqual(el, 0)).to.eql(false);
        });
        ...
        it ('not see a boolean as an integer above or equal', () => {
            let vals = fnBooleanValues();
            for (let el of vals) expect(isIntegerAboveOrEqual(0, el)).to.eql(false);
        });
