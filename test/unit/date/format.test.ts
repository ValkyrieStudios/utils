...
describe('Date - format', () => {
    ...
    it('Throw when passed a non-string for spec', () => {
        for (const el of CONSTANTS.NOT_STRING) {
            expect(
                () => format(new Date(), el)
            ).toThrowError(/format: spec must be a string/);
        }
    });
    ...
    it('Throw when passed a non-string for locale', () => {
        for (const el of CONSTANTS.NOT_STRING) {
            ...
            ).toThrowError(/format: locale must be a string/);
        }
    });
    ...
    describe('setLocale and getLocale', () => {
        ...
        it('Should be a function', () => {
            expect(typeof format.setLocale).toBe('function');
            expect(typeof format.getLocale).toBe('function');
        });

        it('Should not be an async function', () => {
            expect(format.setLocale.constructor.name).not.toBe('AsyncFunction');
            expect(format.getLocale.constructor.name).not.toBe('AsyncFunction');
        });

        it('Should set and return the default locale when a valid locale is provided', () => {
            format.setLocale('fr-FR');
            expect(format.getLocale()).toBe('fr-FR');
        });
        ...
        it('Should throw an error when a non-string value is provided', () => {
            for (const el of CONSTANTS.NOT_STRING_WITH_EMPTY) {
                expect(() => {
                    format.setLocale(el);
                }).toThrowError(/format\/setLocale: locale should be a string/);
            }
        });
    ...
    describe('setZone and getZone', () => {
        ...
        it('Should be a function', () => {
            expect(typeof format.setZone).toBe('function');
            expect(typeof format.getZone).toBe('function');
        });

        it('Should not be an async function', () => {
            expect(format.setZone.constructor.name).not.toBe('AsyncFunction');
            expect(format.getZone.constructor.name).not.toBe('AsyncFunction');
        });

        it('Should set and return the default time zone when a valid zone is provided', () => {
            format.setZone('America/New_York');
            expect(format.getZone()).toBe('America/New_York'); // Ensure the time zone was set correctly
        });

        it('Should throw an error when an invalid time zone is provided', () => {
            expect(() => {
                format.setZone('Invalid/Zone');
            }).toThrowError(/format\/setZone: 'Invalid\/Zone' is not a valid zone/);
        });
        ...
        it('Should throw an error when a non-string value is provided', () => {
            for (const el of CONSTANTS.NOT_STRING) {
                expect(() => {
                    format.setZone(el);
                }).toThrowError(/format\/setZone: zone should be a string/);
            }
        });

        it('Should return the default time zone', () => {
            expect(format.getZone()).toBe(DEFAULT); // Ensure it returns the correct default time zone
        });
    });
    ...
    describe('setStartOfWeek and getStartOfWeek', () => {
        ...
        it('Should be a function', () => {
            expect(typeof format.setStartOfWeek).toBe('function');
            expect(typeof format.getStartOfWeek).toBe('function');
        });

        it('Should not be an async function', () => {
            expect(format.setStartOfWeek.constructor.name).not.toBe('AsyncFunction');
            expect(format.getStartOfWeek.constructor.name).not.toBe('AsyncFunction');
        });
        ...
        it('should set and return the default start of the week when a valid value is provided', () => {
            for (const el of ['mon', 'sat', 'sun']) {
                ...
                expect(format.getStartOfWeek()).toBe(el);
            }
        });
        ...
        it('should throw an error when an invalid start of the week is provided', () => {
            ...
            }).toThrowError(/format\/setStartOfWeek: sow should be a valid start of week/);
        });
        ...
        it('should return the default start of the week', () => {
            expect(format.getStartOfWeek()).toBe('mon');
        });
    });
    ...
    it('Specific Cases: Default locale - UTC - Should format different combinations of tokens correctly', () => {
        for (const el of [
            ...
            expect(format(new Date(el.i), el.s, 'en', 'UTC')).toBe(el.o);
            expect(format(el.i, el.s, 'en', 'UTC')).toBe(el.o);
        }
    });
    ...
    it('Specific Cases: French locale - UTC Should format different combinations of tokens correctly', () => {
        for (const el of [
            ...
            expect(format(new Date(el.i), el.s, 'fr', 'UTC')).toBe(el.o);
            expect(format(el.i, el.s, 'fr', 'UTC')).toBe(el.o);
        }
    });
    ...
    it('Specific Cases: French locale - Different Timezones Should format different combinations of tokens correctly', () => {
        for (const el of [
            ...
            expect(format(new Date(el.i), el.s, 'fr', el.tz)).toBe(el.o);
            expect(format(el.i, el.s, 'fr', el.tz)).toBe(el.o);
        }
    });
    ...
    it('Specific Cases - Dutch locale', () => {
        for (const el of [
            ...
            expect(format(new Date(el.i), el.s, 'nl', el.tz)).toBe(el.o);
            expect(format(el.i, el.s, 'nl', el.tz)).toBe(el.o);
        }
    });
    ...
    describe('token:YYYY', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'YYYY')).toBe(el[1]);
                expect(format(el[0], 'YYYY')).toBe(el[1]);
            }
        });

        it('Should not take locale into account', () => {
            expect(format(new Date('2019-02-01T05:20:19+02:00'), 'YYYY', 'fr')).toBe('2019');
            expect(format('2019-02-01T05:20:19+02:00', 'YYYY', 'fr')).toBe('2019');
        });
    });
    ...
    describe('token:Q', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'Q')).toBe(el[1]);
                expect(format(el[0], 'Q')).toBe(el[1]);
            }
        });

        it('Should not take locale into account', () => {
            expect(format(new Date('2019-02-01T05:20:19+02:00'), 'Q', 'fr')).toBe('1');
            expect(format('2019-02-01T05:20:19+02:00', 'Q', 'fr')).toBe('1');
        });
    });
    ...
    describe('token:MMMM', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MMMM')).toBe(el[1]);
                expect(format(el[0], 'MMMM')).toBe(el[1]);
            }
        });
        ...
        it('Should take locale into account', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MMMM', 'fr')).toBe(el[1]);
                expect(format(el[0], 'MMMM', 'fr')).toBe(el[1]);
            }
            ...
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MMMM', 'nl')).toBe(el[1]);
                expect(format(el[0], 'MMMM', 'nl')).toBe(el[1]);
            }
            ...
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MMMM', 'pt')).toBe(el[1]);
                expect(format(el[0], 'MMMM', 'pt')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:MMM', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MMM')).toBe(el[1]);
                expect(format(el[0], 'MMM')).toBe(el[1]);
            }
        });
        ...
        it('Should take locale into account', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MMM', 'fr')).toBe(el[1]);
                expect(format(el[0], 'MMM', 'fr')).toBe(el[1]);
            }
            ...
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MMM', 'nl')).toBe(el[1]);
                expect(format(el[0], 'MMM', 'nl')).toBe(el[1]);
            }
            ...
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MMM', 'pt')).toBe(el[1]);
                expect(format(el[0], 'MMM', 'pt')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:MM', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MM')).toBe(el[1]);
                expect(format(el[0], 'MM')).toBe(el[1]);
            }
        });
        ...
        it('Should not take locale into account', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'MM', 'fr')).toBe(el[1]);
                expect(format(el[0], 'MM', 'fr')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:M', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'M')).toBe(el[1]);
                expect(format(el[0], 'M')).toBe(el[1]);
            }
        });
        ...
        it('Should not take locale into account', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'M', 'fr')).toBe(el[1]);
                expect(format(el[0], 'M', 'fr')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:WW', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'WW')).toBe(el[1]);
                expect(format(el[0], 'WW')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:W', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'W')).toBe(el[1]);
                expect(format(el[0], 'W')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:WW sow:sun', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'WW', 'en', 'Europe/Brussels', 'sun')).toBe(el[1]);
                expect(format(el[0], 'WW', 'en', 'Europe/Brussels', 'sun')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:W sow:sun', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'W', 'en', 'Europe/Brussels', 'sun')).toBe(el[1]);
                expect(format(el[0], 'W', 'en', 'Europe/Brussels', 'sun')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:WW sow:sat', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'WW', 'en', 'Europe/Brussels', 'sat')).toBe(el[1]);
                expect(format(el[0], 'WW', 'en', 'Europe/Brussels', 'sat')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:W sow:sat', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'W', 'en', 'Europe/Brussels', 'sat')).toBe(el[1]);
                expect(format(el[0], 'W', 'en', 'Europe/Brussels', 'sat')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:DD', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(el[0], 'DD')).toBe(el[1]);
                expect(format(new Date(el[0]), 'DD')).toBe(el[1]);
            }
        });
        ...
        it('Should not take locale into account', () => {
            for (const el of [
                ...
                expect(format(el[0], 'DD', 'fr')).toBe(el[1]);
                expect(format(new Date(el[0]), 'DD', 'fr')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:D', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(el[0], 'D')).toBe(el[1]);
                expect(format(new Date(el[0]), 'D')).toBe(el[1]);
            }
        });
        ...
        it('Should not take locale into account', () => {
            for (const el of [
                ...
                expect(format(el[0], 'D', 'fr')).toBe(el[1]);
                expect(format(new Date(el[0]), 'D', 'fr')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:dddd', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(el[0], 'dddd')).toBe(el[1]);
                expect(format(new Date(el[0]), 'dddd')).toBe(el[1]);
            }
        });
        ...
        it('Should take locale into account', () => {
            for (const el of [
                ...
                expect(format(el[0], 'dddd', 'fr')).toBe(el[1]);
                expect(format(new Date(el[0]), 'dddd', 'fr')).toBe(el[1]);
            }
            ...
            for (const el of [
                ...
                expect(format(el[0], 'dddd', 'nl')).toBe(el[1]);
                expect(format(new Date(el[0]), 'dddd', 'nl')).toBe(el[1]);
            }
            ...
            for (const el of [
                ...
                expect(format(el[0], 'dddd', 'pt')).toBe(el[1]);
                expect(format(new Date(el[0]), 'dddd', 'pt')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:ddd', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(el[0], 'ddd')).toBe(el[1]);
                expect(format(new Date(el[0]), 'ddd')).toBe(el[1]);
            }
        });

        it('Should take zone into account', () => {
            // When converted to Europe/Brussels the day should be next day
            expect(format(new Date('2019-02-25T22:09:09-07:00'), 'ddd', 'en', 'Europe/Brussels')).toBe('Tue');

            // When converted to CST (which is one hour ahead of MST) it should still be same day
            expect(format(new Date('2019-02-25T22:09:09-07:00'), 'ddd', 'en', 'CST')).toBe('Mon');

            // When converted to EST (which is two hours ahead of MST) it should be next day
            expect(format(new Date('2019-02-25T22:09:09-07:00'), 'ddd', 'en', 'EST')).toBe('Tue');

            // When converted to MST (which is the same as the offset in the date) it should be same day
            expect(format(new Date('2019-02-25T22:09:09-07:00'), 'ddd', 'en', 'MST')).toBe('Mon');
        });
        ...
        it('Should take locale into account', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'ddd', 'fr')).toBe(el[1]);
                expect(format(el[0], 'ddd', 'fr')).toBe(el[1]);
            }
            ...
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'ddd', 'nl')).toBe(el[1]);
                expect(format(el[0], 'ddd', 'nl')).toBe(el[1]);
            }
            ...
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'ddd', 'pt')).toBe(el[1]);
                expect(format(el[0], 'ddd', 'pt')).toBe(el[1]);
            }
        });
    });
    ...
    describe('token:HH', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'HH')).toBe(el[1]);
                expect(format(el[0], 'HH')).toBe(el[1]);
            }
        });

        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45+00:00'), 'HH', 'en', 'Europe/Paris')).toBe('09');
            expect(format(new Date('2022-03-06T08:30:45+00:00'), 'HH', 'en', 'America/New_York')).toBe('03');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'HH', 'en', 'UTC')).toBe('22');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'HH', 'en', 'UTC')).toBe('06');
            expect(format('2022-03-06T08:30:45+00:00', 'HH', 'en', 'Europe/Paris')).toBe('09');
            expect(format('2022-03-06T08:30:45+00:00', 'HH', 'en', 'America/New_York')).toBe('03');
            expect(format('2022-03-06T00:30:45+02:00', 'HH', 'en', 'UTC')).toBe('22');
            expect(format('2022-03-06T23:30:45-07:00', 'HH', 'en', 'UTC')).toBe('06');
        });
    });
    ...
    describe('token:H', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'H')).toBe(el[1]);
                expect(format(el[0], 'H')).toBe(el[1]);
            }
        });

        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'H', 'en', 'Europe/Paris')).toBe('9');
            expect(format(new Date('2022-03-06T08:30:45Z'), 'H', 'en', 'America/New_York')).toBe('3');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'H', 'en', 'UTC')).toBe('22');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'H', 'en', 'UTC')).toBe('6');
            expect(format('2022-03-06T08:30:45Z', 'H', 'en', 'Europe/Paris')).toBe('9');
            expect(format('2022-03-06T08:30:45Z', 'H', 'en', 'America/New_York')).toBe('3');
            expect(format('2022-03-06T00:30:45+02:00', 'H', 'en', 'UTC')).toBe('22');
            expect(format('2022-03-06T23:30:45-07:00', 'H', 'en', 'UTC')).toBe('6');
        });
    });
    ...
    describe('token:hh', () => {
        ...
        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'hh', 'en', 'Europe/Paris')).toBe('09');
            expect(format(new Date('2022-03-06T08:30:45Z'), 'hh', 'en', 'America/New_York')).toBe('03');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'hh', 'en', 'UTC')).toBe('10');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'hh', 'en', 'UTC')).toBe('06');
            expect(format('2022-03-06T08:30:45Z', 'hh', 'en', 'Europe/Paris')).toBe('09');
            expect(format('2022-03-06T08:30:45Z', 'hh', 'en', 'America/New_York')).toBe('03');
            expect(format('2022-03-06T00:30:45+02:00', 'hh', 'en', 'UTC')).toBe('10');
            expect(format('2022-03-06T23:30:45-07:00', 'hh', 'en', 'UTC')).toBe('06');
        });

        it('Should handle AM/PM properly', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'hh A')).toBe('09 AM');
            expect(format(new Date('2022-03-06T15:45:00Z'), 'hh A')).toBe('04 PM');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'hh A')).toBe('11 PM');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'hh A')).toBe('07 AM');
            expect(format('2022-03-06T08:30:45Z', 'hh A')).toBe('09 AM');
            expect(format('2022-03-06T15:45:00Z', 'hh A')).toBe('04 PM');
            expect(format('2022-03-06T00:30:45+02:00', 'hh A')).toBe('11 PM');
            expect(format('2022-03-06T23:30:45-07:00', 'hh A')).toBe('07 AM');
        });
    });
    ...
    describe('token:h', () => {
        ...
        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'h', 'en', 'Europe/Paris')).toBe('9');
            expect(format(new Date('2022-03-06T08:30:45Z'), 'h', 'en', 'America/New_York')).toBe('3');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'h', 'en', 'UTC')).toBe('10');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'h', 'en', 'UTC')).toBe('6');
            expect(format('2022-03-06T08:30:45Z', 'h', 'en', 'Europe/Paris')).toBe('9');
            expect(format('2022-03-06T08:30:45Z', 'h', 'en', 'America/New_York')).toBe('3');
            expect(format('2022-03-06T00:30:45+02:00', 'h', 'en', 'UTC')).toBe('10');
            expect(format('2022-03-06T23:30:45-07:00', 'h', 'en', 'UTC')).toBe('6');
        });

        it('Should handle AM/PM properly', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'h A')).toBe('9 AM');
            expect(format(new Date('2022-03-06T15:45:00Z'), 'h A')).toBe('4 PM');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'h A')).toBe('11 PM');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'h A')).toBe('7 AM');
            expect(format('2022-03-06T08:30:45Z', 'h A')).toBe('9 AM');
            expect(format('2022-03-06T15:45:00Z', 'h A')).toBe('4 PM');
            expect(format('2022-03-06T00:30:45+02:00', 'h A')).toBe('11 PM');
            expect(format('2022-03-06T23:30:45-07:00', 'h A')).toBe('7 AM');
        });
    });
    ...
    describe('token:mm', () => {
        ...
        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'mm', 'en', 'Europe/Paris')).toBe('30');
            expect(format(new Date('2022-03-06T08:30:45Z'), 'mm', 'en', 'America/New_York')).toBe('30');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'mm', 'en', 'UTC')).toBe('30');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'mm', 'en', 'UTC')).toBe('30');
            expect(format(new Date('2022-03-06T00:30:45+05:30'), 'mm', 'en', 'Europe/Brussels')).toBe('00'); /* Eg: India: UTC +5:30 */
            expect(format('2022-03-06T08:30:45Z', 'mm', 'en', 'Europe/Paris')).toBe('30');
            expect(format('2022-03-06T08:30:45Z', 'mm', 'en', 'America/New_York')).toBe('30');
            expect(format('2022-03-06T00:30:45+02:00', 'mm', 'en', 'UTC')).toBe('30');
            expect(format('2022-03-06T23:30:45-07:00', 'mm', 'en', 'UTC')).toBe('30');
            expect(format('2022-03-06T00:30:45+05:30', 'mm', 'en', 'Europe/Brussels')).toBe('00'); /* Eg: India: UTC +5:30 */
        });
    });
    ...
    describe('token:m', () => {
        ...
        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'm', 'en', 'Europe/Paris')).toBe('30');
            expect(format(new Date('2022-03-06T08:30:45Z'), 'm', 'en', 'America/New_York')).toBe('30');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'm', 'en', 'UTC')).toBe('30');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'm', 'en', 'UTC')).toBe('30');
            expect(format('2022-03-06T08:30:45Z', 'm', 'en', 'Europe/Paris')).toBe('30');
            expect(format('2022-03-06T08:30:45Z', 'm', 'en', 'America/New_York')).toBe('30');
            expect(format('2022-03-06T00:30:45+02:00', 'm', 'en', 'UTC')).toBe('30');
            expect(format('2022-03-06T23:30:45-07:00', 'm', 'en', 'UTC')).toBe('30');
        });
    });
    ...
    describe('token:ss', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'ss')).toBe(el[1]);
                expect(format(el[0], 'ss')).toBe(el[1]);
            }
        });

        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'ss', 'en', 'Europe/Paris')).toBe('45');
            expect(format(new Date('2022-03-06T08:30:45Z'), 'ss', 'en', 'America/New_York')).toBe('45');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'ss', 'en', 'UTC')).toBe('45');
            expect(format(new Date('2022-03-06T23:30:15-07:00'), 'ss', 'en', 'UTC')).toBe('15');
            expect(format('2022-03-06T08:30:45Z', 'ss', 'en', 'Europe/Paris')).toBe('45');
            expect(format('2022-03-06T08:30:45Z', 'ss', 'en', 'America/New_York')).toBe('45');
            expect(format('2022-03-06T00:30:45+02:00', 'ss', 'en', 'UTC')).toBe('45');
            expect(format('2022-03-06T23:30:15-07:00', 'ss', 'en', 'UTC')).toBe('15');
        });
    });
    ...
    describe('token:s', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 's')).toBe(el[1]);
                expect(format(el[0], 's')).toBe(el[1]);
            }
        });

        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 's', 'en', 'Europe/Paris')).toBe('45');
            expect(format(new Date('2022-03-06T08:30:45Z'), 's', 'en', 'America/New_York')).toBe('45');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 's', 'en', 'UTC')).toBe('45');
            expect(format(new Date('2022-03-06T23:30:15-07:00'), 's', 'en', 'UTC')).toBe('15');
            expect(format('2022-03-06T08:30:45Z', 's', 'en', 'Europe/Paris')).toBe('45');
            expect(format('2022-03-06T08:30:45Z', 's', 'en', 'America/New_York')).toBe('45');
            expect(format('2022-03-06T00:30:45+02:00', 's', 'en', 'UTC')).toBe('45');
            expect(format('2022-03-06T23:30:15-07:00', 's', 'en', 'UTC')).toBe('15');
        });
    });
    ...
    describe('token:SSS', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'SSS')).toBe(el[1]);
                expect(format(el[0], 'SSS')).toBe(el[1]);
            }
        });

        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'SSS', 'en', 'Europe/Paris')).toBe('000');
            expect(format(new Date('2022-03-06T08:30:45.010Z'), 'SSS', 'en', 'Europe/Paris')).toBe('010');
            expect(format(new Date('2022-03-06T08:30:45.123Z'), 'SSS', 'en', 'America/New_York')).toBe('123');
            expect(format(new Date('2022-03-06T00:30:45.987+02:00'), 'SSS', 'en', 'UTC')).toBe('987');
            expect(format(new Date('2022-03-06T23:30:15.001-07:00'), 'SSS', 'en', 'UTC')).toBe('001');
            expect(format('2022-03-06T08:30:45Z', 'SSS', 'en', 'Europe/Paris')).toBe('000');
            expect(format('2022-03-06T08:30:45.123Z', 'SSS', 'en', 'America/New_York')).toBe('123');
            expect(format('2022-03-06T00:30:45.987+02:00', 'SSS', 'en', 'UTC')).toBe('987');
            expect(format('2022-03-06T23:30:15.001-07:00', 'SSS', 'en', 'UTC')).toBe('001');
        });
    });
    ...
    describe('token:A', () => {
        it('Should be correct', () => {
            for (const el of [
                ...
                expect(format(new Date(el[0]), 'A')).toBe(el[1]);
                expect(format(el[0], 'A')).toBe(el[1]);
            }
        });

        it('Should take zone into account', () => {
            expect(format(new Date('2022-03-06T08:30:45Z'), 'A', 'en', 'Europe/Paris')).toBe('AM');
            expect(format(new Date('2022-03-06T08:30:45Z'), 'A', 'en', 'America/New_York')).toBe('AM');
            expect(format(new Date('2022-03-06T00:30:45+02:00'), 'A', 'en', 'UTC')).toBe('PM');
            expect(format(new Date('2022-03-06T23:30:45-07:00'), 'A', 'en', 'UTC')).toBe('AM');
            expect(format('2022-03-06T08:30:45Z', 'A', 'en', 'Europe/Paris')).toBe('AM');
            expect(format('2022-03-06T08:30:45Z', 'A', 'en', 'America/New_York')).toBe('AM');
            expect(format('2022-03-06T00:30:45+02:00', 'A', 'en', 'UTC')).toBe('PM');
            expect(format('2022-03-06T23:30:45-07:00', 'A', 'en', 'UTC')).toBe('AM');
        });
    });
    ...
    describe('token:l', () => {
        it('Should format the date correctly in different locales', () => {
            for (const el of [
                ...
                expect(format(new Date(el.d), 'l', el.locale)).toBe(el.o);
                expect(format(el.d, 'l', el.locale)).toBe(el.o);
            }
        });
    });
    ...
    describe('token:t', () => {
        it('Should format the time correctly in different locales', () => {
            for (const el of [
                ...
                expect(format(new Date(el.d), 't', el.locale, 'UTC')).toBe(el.o);
                expect(format(el.d, 't', el.locale, 'UTC')).toBe(el.o);
            }
        });
    });
