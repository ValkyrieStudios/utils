...
describe('Array - join', () => {
    ...
    describe('set', () => {
        ...
        it('does not autotrims strings when joining if option is turned off', () => {
            assert.equal(
                join(new Set(['   valkyrie ', '   studios  ']), {valtrim: false}),
                'valkyrie     studios'
            );
        });
        ...
        it('does not autotrims strings when joining and after joining if option is turned off', () => {
            assert.equal(
                join(new Set(['   valkyrie ', '   studios  ']), {valtrim: false, trim: false}),
                '   valkyrie     studios  '
            );
        });
        ...
        it('allows you to override the delimiter with an empty string when joining with trimming turned off', () => {
            assert.equal(
                join(new Set(['   valkyrie ', '   studios  ']), {delim: '', valtrim: false, trim: false}),
                '   valkyrie    studios  '
            );
        });
        ...
        it('allows you to override the delimiter with an empty string when joining with trimming turned on', () => {
            assert.equal(
                join(new Set(['   valkyrie ', '   studios  ']), {delim: ''}),
                'valkyriestudios'
            );
        });
        ...
        it('allows you to join a mix of numbers and strings', () => {
            assert.equal(
                join(new Set(['   valkyrie ', 569.45, '   studios  ']), {delim: '@'}),
                'valkyrie@569.45@studios'
            );
        });
        ...
        it('allows you to join only numbers', () => {
            assert.equal(
                join(new Set([569.45, 965.12]), {delim: '@', valround: 0}),
                '569@965'
            );
        });
        ...
        it('allows you to turn on innertrim to trim inside of the strings as well', () => {
            assert.equal(
                join(new Set(['   hello     world    ', ' this    is', '   peter   from    valkyrie']), {delim: ' ', innertrim: true}),
                'hello world this is peter from valkyrie'
            );
        });

        it('allows you to join a mix of numbers and strings and autoround to a certain precision', () => {
            assert.equal(join(new Set(['   valkyrie ', 569.45, '   studios  ']), {delim: '@', valround: 0}), 'valkyrie@569@studios');
            assert.equal(join(new Set(['   valkyrie ', 569.45, '   studios  ']), {delim: '@', valround: 1}), 'valkyrie@569.5@studios');
            assert.equal(join(new Set(['   valkyrie ', 569.45, '   studios  ']), {delim: '@', valround: 2}), 'valkyrie@569.45@studios');
            assert.equal(join(new Set(['   valkyrie ', 569.45, '   studios  ']), {delim: '@', valround: 3}), 'valkyrie@569.45@studios');
        });

        it('removes multiple invalid values while joining', () => {
            assert.equal(join(new Set([false, false, 'hi', false, false, 'there']), {delim: '|'}), 'hi|there');
        });

        it('allows you to dedupe values while joining', () => {
            assert.equal(join(new Set(['prop_1', 'prop_2', 'prop_1', 'prop_3']), {
                delim: ',',
                dedupe: true,
            }), 'prop_1,prop_2,prop_3');
        });
        ...
        it('allows you to dedupe after trimming values while joining', () => {
            assert.equal(join(new Set(['prop_1', 'prop_2', ' prop_1', 'prop_3 ', ' prop_3']), {
                delim: ',',
                dedupe: true,
            }), 'prop_1,prop_2,prop_3');
        });
        ...
        it('allows you to dedupe after rounding values while joining', () => {
            assert.equal(join(new Set([58.432, 58.43, 58.421]), {delim: '@', dedupe: true, valround: 1}), '58.4');
        });

        it('allows you to dedupe after rounding values while joining and dedupe after stringification', () => {
            assert.equal(join(new Set([58.432, 58.43, 58.421, '58', 59.1, '59']), {delim: '@', dedupe: true, valround: 0}), '58@59');
        });
    });
});
