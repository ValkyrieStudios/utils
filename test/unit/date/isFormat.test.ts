...
describe('Date - isDateFormat', () => {
    it('Return false when passed a non-string for val', () => {
        for (const el of CONSTANTS.NOT_STRING) {
            expect(isDateFormat(el, 'YYYY-MM-DD')).toBe(false);
        }
    });
    ...
    it('Throw when passed a non-string for spec', () => {
        for (const el of CONSTANTS.NOT_STRING) {
            ...
            ).toThrowError(/isDateFormat: spec must be a string/);
        }
    });
    ...
    it('Should throw when passed a spec with incorrect brackets', () => {
        ...
        ).toThrowError(/isDateFormat: Unmatched \[ in format string/);
    });
    ...
    it('Should return false if the spec contains no valid tokens', () => {
        for (const {input, spec} of [
            ...
            expect(isDateFormat(input, spec)).toBe(false);
        }
    });
    ...
    it('Should correctly handle values valid to the spec', () => {
        for (const {input, spec} of [
            ...
            expect(isDateFormat(input, spec), `Expected ${input} to match spec ${spec}`).toBe(true);
        }
    });
    ...
    it('Should correctly handle values invalid to the spec', () => {
        for (const {input, spec} of [
            ...
            expect(isDateFormat(input, spec), `Expected ${input} not to match spec ${spec}`).toBe(false);
        }
    });
